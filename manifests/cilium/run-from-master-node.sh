...
ssh -o ConnectTimeout=10 "${SSH_REMOTE_USER}@192.168.2.19" 'sudo systemctl start k3s' 2>&1 || echo "macmini7: не вдалося"
ssh -o ConnectTimeout=10 "${SSH_REMOTE_USER}@192.168.1.19" 'sudo systemctl start k3s' 2>&1 || echo "beelinkeqr5: не вдалося"
...
# systemctl status повертає ненульовий код, якщо юніт не active — не валити скрипт при set -e / pipefail
sudo systemctl status k3s --no-pager 2>&1 | head -15 || true
